// script.js - Lannefy Professional Logic Engine (V5 - Firebase Cloud & Slider Fix)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- 5. PROGRESS BAR & INTEGRASI FIREBASE CLOUD ---
    ...
    function startProcessing() {
        ...
        const interval = setInterval(() => {
            ...
            if(percent < 30) {
                ...
                progressText.textContent = 'Menghubungkan ke Cloud Server...';
            } else {
                // Warna Akhir (Success)
            ...
            if(timer <= 0) {
                ...
                // Eksekusi pengiriman data ke Firebase
                if (typeof firebase !== 'undefined') {
                    firebase.database().ref('orders/' + orderData.invoice).set(orderData)
                    .then(() => {
                        console.log("Mantap! Data pesanan sukses masuk ke Firebase Database!");
                        window.location.href = 'pay.html';
                    })
                    .catch((error) => {
                        console.error("Waduh, gagal simpan ke Firebase: ", error);
                        // Tetap dialihkan ke pay.html agar pelanggan tetap bisa bayar
                        window.location.href = 'pay.html';
                    });
                } else {
                    console.warn("Script Firebase belum dipasang di index.html, dialihkan secara offline.");
                    window.location.href = 'pay.html';
                }
            }
        }, 100);
    ...
    // --- 7. LOGIKA SLIDER TESTIMONI (FIXED) ---
    const slider = document.querySelector('.ig-slider');
    const slides = document.querySelectorAll('.ig-slide-img');
    ...
    if (slider && slides.length > 0) {
        ...
        // Fungsi memindahkan gambar
        function updateSlider() {
            // Menggeser posisi X sesuai indeks gambar
        ...
        nextBtn.addEventListener('click', () => {
            if (currentIndex < slides.length - 1) {
                ...
                currentIndex = 0; // Balik ke awal kalau sudah mentok
            }
            updateSlider();
        });
        ...
        prevBtn.addEventListener('click', () => {
            if (currentIndex > 0) {
                ...
                currentIndex = slides.length - 1; // Lompat ke akhir kalau di posisi awal
            }
            updateSlider();
        });

        // Fitur Swipe (Geser Pakai Jari di Layar HP)
        let touchStartX = 0;
        let touchEndX = 0;
        ...
        function handleSwipe() {
            ...
            // Jika digeser ke kiri lebih dari 40px
            if (swipeDistance > 40) {
                nextBtn.click();
            }
            // Jika digeser ke kanan lebih dari 40px
